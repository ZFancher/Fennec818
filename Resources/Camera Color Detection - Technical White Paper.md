...
Each displayed percentage is the fraction of pixels inside the polygon that belong to that Munsell cluster, rounded independently to the nearest integer. Features rounding to 0% are suppressed as true k-means noise at this scale. Features at 1% are retained and reported. This is significant because SoilAnalyzeCapture results are written to the soil horizon as separate redox rows, and ADS Plus evaluates indicator thresholds by summing percentages across all qualifying rows in the same horizon — two 1% features therefore correctly contribute together to meet a ≥2% threshold. Suppressing 1% features would silently prevent that inference from firing in valid cases.
